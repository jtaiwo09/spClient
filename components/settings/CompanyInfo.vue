<template>
  <div class="s-flex s-flex-col s-h-[90%] sm:s-h-full">
    <div class="s-max-w-[40rem] s-mx-5 s-mt-4">
      <p class="s-text-[12px] s-font-[700] s-mb-2">
        ARE YOU LISTING AS A COMPANY
      </p>
      <div class="s-flex s-mb-8">
        <div class="s-mr-4">
          <CustomBox
            v-model="isListingCompany"
            label="Yes"
            name="Yes"
            :val="'yes'"
            type="radio"
      ...
      <div class="s-py-4 s-mx-5 s-border-b s-border-solid s-border-[#e5e5e5]">
        <h3 class="s-font-semibold ">Alternative Contact Details</h3>
      </div>

      <div class="s-max-w-[40rem] s-mx-5 s-mt-12">
...
export default {
  ...
  methods: {
    ...mapActions('user', ['updateAlternativeContact']),
    ...
    updateForm() {
      ...
      if (this.userData && this.userData.alternativeContact) {
        ...
        this.isListingCompany = listingAsCompany ? "yes" : "no"
      }
    },
    ...
    async updateProfile() {
      ...
      if (this.$v.$invalid) {
        ...
        const data = {
          ...
          listingAsCompany: this.isListingCompany  === 'yes',
          companyName: this.formData.firstname + " " + this.formData.lastname
        }
        try {
          const response = await this.updateAlternativeContact(data)
          this.loading = false
