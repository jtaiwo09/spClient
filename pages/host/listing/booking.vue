<template>
  <div class="sm:s-relative">
    ...
    <Subscription v-if="currentLevel === 1" />
    <Pricing v-if="currentLevel === 2" />
    <Discount v-if="currentLevel === 3 && addDiscount" />
    <ServiceCharge v-if="currentLevel === 4" />
    <HouseRules v-if="currentLevel === 5" />
    <PriceSummary v-if="currentLevel === 6" />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState('host/listing', ['currentLevel', 'step', 'addDiscount']),
    progressBar() {
      if (this.step === 2) {
